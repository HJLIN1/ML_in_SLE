#請示範一個以羅吉斯回歸為架構，使用PBMC data預測是否罹患SLE的模型，
#將資料分成訓練、驗證、測試資料，使用梯度下降法，並在訓練過程中利用特徵選擇等方式避免overfitting，得出最佳之模型

## 使用L1正規化的羅吉斯回歸模型，並使用SelectFromModel進行特徵選擇
import pandas as pd
import numpy as np
...
# n_samples, n_features = df_Tx.shape
# X = df_Tx
# missing_rate = 0.5
...
# X_missing = X.copy()
# # X_missing[missing_samples == 1, missing_features] = np.nan

# # 填補缺失值
# imputers = [('BayesianRidge', BayesianRidge()),
...
# for imputer_name, imputer_model in imputers:
#     imputer = IterativeImputer(random_state=0, estimator=imputer_model)
#     X_imputed = imputer.fit_transform(X_missing)
#     rmse = sqrt(mean_squared_error(X, X_imputed))
#     # lr_model = LinearRegression()
#     # lr_model.fit(X_imputed, df_Ty)
#     # y_pred_lr = lr_model.predict(X_imputed)
#     # rmse_lr = sqrt(mean_squared_error(df_Ty, y_pred_lr))
#     print(f"{imputer_name} RMSE: {rmse}")
...
X = df_Tx_filled    
y = np.ravel(df_Ty)  
# df_Tx_filled是填補完整後(每個特徵都具有完整的觀測值)，未進行特徵選擇、資料標準化的完整dataframe! 
...
# 特徵選擇
selector = SelectFromModel(clf, max_features=20)  #"l1" 19-65 皆相同結果 0.8485/0.8529 / "L2" 19最好 0.8485/0.8824 ; 20後 0.8182/0.8529
selector.fit(X_train, y_train)
# 獲取被選擇的特徵
selected_features = df_Tx_filled.columns[selector.get_support()]
decision = selector.get_support()  #selector.get_support()呈現出各個特徵是否被選擇的布林值1D ndarray
...
# # 使用 clf.feature_importances_ 屬性來取得每個特徵的重要性，並使用 Matplotlib 或其他視覺化套件將其視覺化。
...
## 用使用者的角度（給最重要的參數
weights = np.abs(clf.coef_).ravel()  # 因為clf.coef_是2D-array(1, 20)，所以...要用revel將ndarray降維成(20,)
weights1 = clf.coef_.ravel()    # 考量原始正負值!
...
top_5_names = selected_features[top_5]
...
# zip1 = zip(top_5_names, top_5_importances1)
# print(len(zip1))
# print(zip1[1])  #為何會出現error，zip物件沒有len、引數!?
# for name, importances1 in zip(top_5_names, top_5_importances1):  #??
#     print(f"{name}: {importances1}")   